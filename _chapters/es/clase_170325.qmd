...
### Sigma-Álgebras y Preliminares de Teoría de la Medida
...
### Espacio de Probabilidad (Definición Formal)
...
* **Discreta:** Si $X$ toma a lo sumo valores numerables, definimos la **función de masa de probabilidad (fmp)**
...
* **Continua:** Si existe una función $f_X$ tal que
  ...
  para todos los conjuntos de Borel $B$, entonces $f_X$ es la **función de densidad de probabilidad (fdp)** de $X$. Debe satisfacer $f_X(x) \geq 0$ y
...
Sea $X$ con fdp $f_X(x) = \lambda e^{-\lambda x}$ para $x \geq 0$ ($\lambda > 0$).
