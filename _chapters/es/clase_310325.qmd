...
La **función de distribución acumulativa (FDA)** de $X$ se define por
...
Una variable aleatoria $X$ es **discreta** si su rango es un conjunto finito o numerable $S \subset \mathbb{R}$. La **función de masa de probabilidad (fmp)** es
...
Una variable aleatoria $X$ es **continua** si existe una **función de densidad de probabilidad (fdp)** $f_X:\mathbb{R}\to[0,\infty)$ tal que, para todo $a < b$,
...
**Observación**: La existencia de una fdp requiere que $F_X$ sea absolutamente continua [@Durrett2019, Sec. 1.3].
...
Dada una variable aleatoria $X$ con fdp $f_X$ y una función $h:\mathbb{R}\to\mathbb{R}$ medible, invertible y diferenciable, definamos $Y = h(X)$. La fdp de $Y$ es
...
Si existe una fdp conjunta $f_{\mathbf{X}}$, entonces
...
**Marginales y Condicionales**: La fdp marginal de $X_1$ es
...
**Ejemplo: Normal Bivariada.** Sea $(X_1, X_2)$ conjuntamente normal con media cero, varianzas $1$, y covarianza $\rho$. La fdp conjunta es
...
1. Demostrar que para cualquier FDA $F$, la función es continua por la derecha y satisface los límites establecidos en $\pm\infty$.
2. Sea $X$ uniforme en $[0,1]$, y $Y = -\ln(1-X)$. Mostrar que $Y$ es exponencial con parámetro $1$.
3. Dadas las variables aleatorias $X$ e $Y$ con fdp conjunta $f(x,y) = 6xy$ para $0 < x < 1$, $0 < y < 1$, $x+y < 1$, calcular $P(X < Y)$ y verificar si $X$ e $Y$ son independientes.