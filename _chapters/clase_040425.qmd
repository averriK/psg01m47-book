...
**Advanced Example**.
Suppose $X$ is a **finite Gaussian mixture**:
...
**Example.**
Let $Z \sim N(0,1)$. Find the distribution of $X = Z^2$.
...
**Example.**
Let $\bar{X} \sim N_2(\mu, \Sigma)$ and
...
**Advanced Example.**
Consider the mapping from polar to Cartesian coordinates: $X = R \cos \Theta$, $Y = R \sin \Theta$, with $R, \Theta$ independent, $R>0$, $\Theta \in (0,2\pi)$. The Jacobian is $r$, giving the joint density $f_{X,Y}(x,y) = f_{R,\Theta}(\sqrt{x^2 + y^2}, \arctan(y/x)) \cdot \frac{1}{\sqrt{x^2 + y^2}}$.
...
**Numerical Example.**
Let $\Sigma = \begin{pmatrix} 2 & 1 \ 1 & 2 \end{pmatrix}$.
Eigenvalues: $3, 1$.
